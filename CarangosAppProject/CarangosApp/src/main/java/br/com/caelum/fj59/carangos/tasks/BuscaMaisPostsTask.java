...
public class BuscaMaisPostsTask extends AsyncTask<Pagina, Void, List<BlogPost>> {
    ...
    protected void onPostExecute(List<BlogPost> retorno) {
        ...
        if (retorno!=null) {
            EventoBlogPostsRecebidos.notificaPostRecebidos(this.application, retorno, true);
        } else {
            EventoBlogPostsRecebidos.notificaPostRecebidos(this.application, retorno, false);
        }
